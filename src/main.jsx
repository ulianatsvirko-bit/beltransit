...
// Parses a minimal `[label](url)` link syntax inside plain FAQ answer strings so
// generate-faq-schema.mjs (which evaluates these arrays as plain JS via vm, not
// JSX) can keep treating answers as ordinary strings, while the UI still renders
...
const services = [
  ...
  },
  {
    icon: ShieldCheck,
    title: "Всё под ключ",
    text: "Выкуп у поставщика + доставка + таможня + сертификация. Один договор — мы берём всё на себя.",
    href: "#request",
    featured: true,
  },
];
...
const deliveryCases = [
  ...
  },
  {
    ...
    facts: ["8 дней", "полная фура", "сезон"],
  },
  {
  ...
  },
  {
    ...
    solution: "Все три поставщика отгружают на наш склад в Вильнюсе, раз в две недели одна машина.",
  ...
  {
    category: "Полная фура",
    title: "Косметика из Европы — клиент сам выкупил, мы только доставили",
    client: "Дистрибьютор косметики, Москва",
    task: "Клиент самостоятельно выкупил товар — 4 паллеты гиалуроновой кислоты. Нужна была только доставка с растаможкой.",
    solution: "Забрали груз со склада поставщика, провезли через таможню от своего имени, доставили до склада в Москве.",
...
const groupageAudiences = [
  {
    icon: ShoppingCart,
    title: "Маркетплейсы (WB, Ozon, Авито)",
...
const groupageAdvantages = [
  ...
  "Машина раз в неделю — не ждёте месяц",
  "Берём от 20 кг — не отказываем малому и среднему бизнесу",
...
const buyoutAdvantages = [
  ...
  "За логистику платите только после доставки до склада в РФ",
  "Все документы на груз передаём вам",
  "Работаем официально — свой склад в Европе, своё юрлицо",
...
const customsStats = [
  ...
  ["день в день", "выпуск груза"],
  ["под ключ", "все документы включены"],
  ["до 40%", "экономия на таможенных платежах"],
];
...
const customsPainPoints = [
  ...
  {
    icon: Zap,
    title: "Выпуск день в день",
    text: "Являемся таможенным оператором — груз, прибывший до 18:00, выпускаем в тот же день.",
  },
];
...
const customsServices = [
  {
    ...
    text: "Подаём декларацию от своего лица — вы не вникаете в процесс.",
  },
  {
    icon: CreditCard,
  ...
  },
  {
    icon: Check,
    title: "Выпуск",
    text: "День в день. Груз, прибывший до 18:00, выпускаем в тот же день — всегда.",
  },
];

const customsSteps = [
  ["1", "Расчёт", "Считаем пошлины и НДС до отправки"],
  ["2", "Документы", "Сами готовим весь пакет документов"],
  ["3", "Декларация", "Подаём от своего лица — вы не вникаете"],
  ["4", "Выпуск", "День в день — груз не стоит на границе"],
  ["5", "Доставка", "Москва — до двери; другие города РФ — до склада в Москве"],
];

const customsAdvantages = [
  "Являемся таможенным оператором — это ускоряет процесс и даёт льготы",
  "Экономия до 40% на таможенных платежах — законные схемы",
  "Считаем платежи до отправки — никаких сюрпризов на границе",
  "Правильный код ТНВЭД — минимальная легальная пошлина",
  "Опыт с фито и ветгрузами — берём сложные категории",
  "Принимаем груз уже выкупленный вами — только доставим и растаможим",
];
...
const cargoCatalog = [
  ...
  },
  {
    ...
    text: "Холодильники, стиральные машины, мелкая техника. Популярный товар для маркетплейсов.",
  },
  {
    icon: Wrench,
  ...
  {
    icon: ShieldCheck,
    title: "Бренды, ушедшие с рынка РФ",
    text: "Continental, Michelin, Kärcher, Bosch и другие бренды остановили прямые продажи — мы их везём через наши европейские юрлица легально.",
  },
  {
    icon: Package,
    title: "Другие товары",
...
const cargoSchemes = [
  ["Объём", "от 20 кг", "от 10 тонн", "любой"],
  ["Срок", "10-14 дней", "7-10 дней", "10-16 дней"],
...
const partnerBenefits = [
  ...
  },
  {
    icon: ClipboardCheck,
    title: "Таможня",
    text: "Оформляем официально — как таможенный оператор с минимальными платежами.",
  },
  {
    icon: Truck,
    title: "Доставка",
...
const warehouseStats = [
  ["Вильнюс", "ближайшая точка ЕС к России"],
...
const warehouseReasons = [
  {
    ...
    text: "Вильнюс стоит на единственном сухопутном коридоре ЕС → Беларусь → Россия. Груз идёт одним маршрутом, без лишних перевалок и рисков. Со склада фура идёт через Беларусь прямо в Россию — никаких лишних границ.",
  },
  {
    icon: Globe2,
    title: "Внутри ЕС — покупка без НДС",
    text: "Любой поставщик из Германии, Польши, Италии или Франции отправляет на наш склад без таможни. При экспорте из своей страны поставщик отгружает по цене НЕТО — без НДС (–20%). Это прямая экономия для вас.",
  },
];
...
const warehouseAudiences = [
  ...
  },
  {
    ...
    text: "Поставщик отгружает партиями — мы храним на складе. Машина уходит раз в неделю: максимальное ожидание понятно заранее.",
  },
  {
    icon: Send,
...
const sanctionsCase = {
  category: "Санкционные грузы",
  title: "Промышленный компонент из Италии — поставщик отказывался работать с РФ",
  client: "Производственная компания, Санкт-Петербург",
  task: "Итальянский завод не продавал напрямую российскому покупателю, а оплата из России не проходила.",
  solution:
    "Провели переговоры и выкупили оборудование от лица нашей европейской компании, оплата прошла в рублях на счёт в российском банке.",
  result:
    "Оборудование выкуплено с 0% НДС и доставлено за 24 дня — легально, с полным пакетом документов на груз.",
  quote:
    "Нам напрямую отказали три поставщика. Здесь оформили сделку на европейское юрлицо — и компонент приехал без нашего следа.",
...
const sanctionsFaq = [
  ...
  [
    ...
    "Выкупаем товар на наше иностранное юрлицо (ЕС, ОАЭ, Сингапур) с 0% НДС. Вы платите в рублях на наш счёт в российском банке, поставщик получает оплату без российского следа. Какие схемы оплаты вообще работают сейчас — смотрите [в отдельном разборе](/blog/oplata-postavshchika-iz-rossii/).",
  ],
  [
...
const tireCargoGroups = [
  {
    ...
    items: [
      "Michelin, Continental, Pirelli, Bridgestone, Hankook, Nokian, Goodyear",
      "Легковые, грузовые, коммерческие, для спецтехники",
      "Всесезонные, летние, зимние — любые размеры под заказ",
      "Оптовые партии для магазинов, шиномонтажей и продажи на Авито",
    ],
  },
  {
    icon: "🔧",
...
const tireAudiences = [
  ...
  {
    icon: "📦",
    title: "Маркетплейсы и Авито",
    text: "Продаёте шины и запчасти на WB, Ozon или Авито — организуем регулярные поставки с минимальными сроками. Авито — особенно популярная площадка для шин.",
  },
];
...
const applianceStats = [
  ...
  ["от 20 кг", "минимальный объём"],
  ["10-14 дней", "срок доставки"],
  ["под ключ", "выкуп + доставка + таможня"],
];

const applianceCargo = [
  { icon: "🖥️", title: "Крупная техника", text: "Bosch, Siemens, Miele, Liebherr — холодильники, стиральные машины, посудомоечные машины" },
  { icon: "📱", title: "Мелкая техника", text: "DeLonghi, Philips, Braun — кофемашины, пылесосы, утюги, блендеры" },
...
const supplierSearchServices = [
  [Search, "Поиск", "находим производителей и дистрибьюторов под ваш товар"],
  [ShieldCheck, "Проверка", "проверяем компанию на надёжность и реальность"],
  [MessageCircle, "Переговоры", "связываемся от вашего имени, узнаём условия"],
  [CreditCard, "Цены", "получаем реальные прайсы и условия поставки"],
  [ClipboardCheck, "Отчёт", "передаём вам список проверенных поставщиков с контактами и ценами"],
  [Handshake, "Введение", "при необходимости представляем вас поставщику лично"],
];
...
function HeroTicker() {
  const items = [
    ...
    ["санкционные грузы", "выкуп без российского следа"],
      ...
      <div className="audience-paths">
        <article className="path-card path-card-buyer">
          ...
          <ul className="path-services-list">
            ...
            <li><Check size={14} /> Доставка санкционных грузов</li>
            <li><Check size={14} /> Таможенное оформление</li>
            <li><Check size={14} /> Сертификация товаров</li>
          </ul>
...
function Process() {
  const steps = [
    ...
    ["4", "Доставка", "Москва — до двери; другие города РФ — до склада в Москве"],
  ];
  ...
  return (
    <section className="section cargo-section">
      <div className="section-heading">
        ...
        <h2>Везём всё что продаётся на рынках и маркетплейсах</h2>
      </div>
  ...
  return (
    <section className="section why-section">
      ...
      <div className="advantage-list">
        {advantages.map((item) => (
          <div className="advantage-item" key={item.text}>
            <span>{item.icon}</span>
            <p>{item.text}</p>
          </div>
        ))}
    ...
    <section className="page-hero cargo-catalog-hero">
      <div className="page-hero-shell">
        <div className="page-hero-inner">
          ...
          <h1>Везём всё что продаётся на рынках, в магазинах и на маркетплейсах</h1>
          <p>
            Сборные грузы, выкуп, санкционные грузы — для любого товара найдём оптимальную схему доставки
            из Европы в Россию.
          </p>
...
function SanctionsHero() {
  return (
    <section className="page-hero fulltruck-hero">
      <div className="page-hero-shell">
        <div className="page-hero-inner">
          <span className="eyebrow">Параллельный импорт · Европа → Россия</span>
          <h1>Доставка санкционных грузов из Европы в Россию</h1>
          <p>
            Выкупаем и доставляем товары у компаний, которые не работают с РФ, — это и есть
            параллельный импорт на практике. Ведём переговоры от лица европейского юрлица, платим
            поставщику без российского следа и везём груз в Россию — легально и без задержек.
          </p>
...
function SanctionsCase() {
  return <FeaturedCaseBlock item={sanctionsCase} heading="Пример доставки санкционного груза" sectionClass="fulltruck-case-section" />;
}
...
function PricingBreakdownExample() {
  return (
    <section className="section">
      <div className="section-heading">
        <span className="eyebrow">Пример расчёта</span>
        <h2>Из чего складывается итоговая стоимость под ключ</h2>
      </div>
      <p className="pricing-breakdown-intro">
        Разберём на примере: партия бытовой техники, 150 кг, стоимость товара по инвойсу — 4 000 €,
        везём сборным грузом из Германии через склад в Вильнюсе с выкупом у поставщика.
      </p>
      <ArticleTable
        headers={["Статья расходов", "Сумма"]}
        rows={[
          ["Стоимость товара (инвойс поставщика)", "4 000 €"],
          ["Доставка Германия → Вильнюс → Москва (сборный груз)", "≈ 450 €"],
          ["Таможенная пошлина (ставка зависит от кода ТН ВЭД, здесь ~10%)", "≈ 400 €"],
          ["НДС 20% от стоимости товара + пошлины", "≈ 880 €"],
          ["Итого «под ключ» — доставка, растаможка, документы", "≈ 5 730 €"],
        ]}
      />
      <p className="pricing-breakdown-note">
        Точные цифры зависят от кода ТН ВЭД вашего товара, схемы закупки (прямая или через выкуп)
        и маршрута. Это ориентир, чтобы понимать порядок цифр — точный расчёт под вашу партию
        готовим за 2 часа после заявки.
      </p>
    </section>
  );
}
      ...
      <div className="page-hero-shell">
        <div className="page-hero-inner">
          ...
          <h1>
            ...
            <span className="accent">для магазинов и маркетплейсов</span>
          </h1>
          <p>
...
function Breadcrumbs({ items }) {
  const parentItem = items.length >= 2
    ...
    : { label: "Главная", href: "/" };
  const backHref = parentItem.href || "/";

  return (
    <nav className="breadcrumbs" aria-label="Навигация по сайту">
      <a className="breadcrumb-back" href={backHref}>
        <ArrowLeft size={13} /> {parentItem.label}
      </a>
